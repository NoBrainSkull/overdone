...
// Loading assets module by impl trait AssetLoader ----------------------------------------------------------------
...
impl AssetLoader for TiledLoader {
    fn load<'a>(
        ...
        Box::pin(async move {
            let root_dir = std::env::current_dir().unwrap().join("assets");
            let path = root_dir.join(load_context.path());
            println!("{:?}", load_context.path());
            ...
            for tileset in &map.tilesets {
                ...
                let texture = load_context.get_handle(asset_path.clone());
                let last_gid = tileset.first_gid + tileset.tilecount.unwrap_or(1);

                for i in tileset.first_gid..last_gid {
            ...
            let loaded_asset = LoadedAsset::new(tiled_map::TiledMap {
                map,
                tilesets: handles,
            });
...
pub fn process_loaded_tile_maps(
    ...
    mut map_events: EventReader<AssetEvent<tiled_map::TiledMap>>,
    maps: Res<Assets<tiled_map::TiledMap>>,
    mut meshes: ResMut<Assets<Mesh>>,
    mut query: Query<(Entity, &Handle<tiled_map::TiledMap>, &mut Map)>,
    new_maps: Query<&Handle<tiled_map::TiledMap>, Added<Handle<tiled_map::TiledMap>>>,
    layer_query: Query<&Layer>,
    chunk_query: Query<&Chunk>,
) {
    let mut changed_maps = Vec::<Handle<tiled_map::TiledMap>>::default();
